@ECHO OFF
ECHO Compiling and running LicenseTest
...
ECHO
javac -d bin -cp libraries\testng.jar ..\eost\License.java LicenseTest.java
...
java -cp libraries/testng.jar;libraries/jcommander.jar;libraries/guice.jar;bin;bin org.testng.TestNG -testclass eost.LicenseTest -log 2 

@ECHO OFF
ECHO Running SC tests for assignment with statement coverage measurement
del jacoco.exec >nul 2>&1
java -ea -javaagent:libraries/jacocoagent.jar=append=false -cp libraries/testng.jar;libraries/jcommander.jar;libraries/guice.jar;libraries/jacocoagent.jar;bin org.testng.TestNG -testclass eost.LicenseTest
